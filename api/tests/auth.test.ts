...
describe('session cookie', () => {
  it('uses a lax cookie for https sign-ins', () => {
    const cookie = createSessionCookie('session_123', 'https://indkobsvogn-web.pages.dev/api/auth/sign-in/verify', Date.now() + 1000)
  ...
  it('uses the configured cookie name when provided', () => {
    const cookie = createSessionCookie(
      'session_123',
      'https://indkobsvogn-web.pages.dev/api/auth/sign-in/verify',
      Date.now() + 1000,
      'custom_session',
    )
